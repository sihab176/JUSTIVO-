...
const RecentNews = () => {
  ...
  return (
    <section className="max-w-7xl mx-auto px-6 py-20 bg-white ">
      {/* Header */}
      <div className="flex items-center gap-4 mb-16 border-b border-gray-100 pb-4">
        <span className="text-[10px] uppercase tracking-[0.3em] text-gray-400">News</span>
          ...
          <div key={news.id} className="group cursor-pointer">
            ...
            <div className="overflow-hidden mb-6">
              <img 
                src={news.image} 
                alt={news.title} 
                className="w-full h-72 object-cover grayscale-[0.2] group-hover:grayscale-0 transition-all duration-500 transform group-hover:scale-105"
              />
            </div>

            {/* Content & Action */}
            <div className="flex items-start justify-between gap-4">
              <h3 className={`text-lg leading-snug transition-colors duration-300 ${
                news.isPrimary ? "text-[#C18C72] underline underline-offset-8" : "text-gray-700 hover:text-[#C18C72]"
              }`}>
                {news.title}
              </h3>
              ...
              <div className={`flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center border transition-all duration-300 ${
                ...
                : "border-gray-200 text-gray-400 group-hover:border-[#C18C72] group-hover:text-[#C18C72]"
              }`}>
